                                          ...
                                          IndexUpdateCheck status)
{
    ...
    switch (status) {
      ...
      case IndexUpdateCheck::Missing:
          clog << format("index file {} does not exist; building it",
                         pair.index_filename)
               << endl;
          break;
      case IndexUpdateCheck::TooOld:
          clog
              << format(
                     "index file {} is older than trace file {}; rebuilding it",
                     pair.index_filename, pair.tarmac_filename)
              << endl;
          break;
      case IndexUpdateCheck::WrongFormat:
          clog << format("index file {} was not generated by this version of "
                         "the tool; rebuilding it",
                         pair.index_filename)
               << endl;
          break;
      case IndexUpdateCheck::Incomplete:
          clog << format("previous generation of index file {} was not "
                         "completed; rebuilding it",
                         pair.index_filename)
               << endl;
          break;
      case IndexUpdateCheck::OK:
          clog << format("index file {} looks ok; not rebuilding it",
                         pair.index_filename)
               << endl;
          break;
...
void CommandLineReporter::indexing_progress(streampos pos)
{
    ...
    if (percentage != last_shown_progress_percentage) {
        ...
        clog << "\r" << format("Reading trace file ({}%)", percentage);
        clog.flush();
    }
}
...
void CommandLineReporter::indexing_done()
{
    ...
    clog << "\r" << "Reading trace file (finished)" << endl;
}
